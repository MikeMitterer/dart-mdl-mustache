...
class Renderer extends Visitor {
    ...
    void render(final List<Node> nodes) {
    ...
    void visitPartial(final PartialNode node) {
        var partialName = node.name;
        Template template = partialResolver == null ? null : partialResolver(partialName);
    ...
    _getNamedProperty(object, name) {
        ...
        var invocation = null;
        try {
            final InstanceMirror instance = mustache.reflect(object);
    ...
    TemplateException _error(String message, Node node) =>
